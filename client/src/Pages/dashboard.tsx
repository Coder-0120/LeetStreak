...
const Dashboard: React.FC = () => {
  ...
  // ── On mount: fetch current user from JWT cookie, then load all data ──
  useEffect(() => {
    const init = async () => {
      try {
    ...
    <div style={s.root}>
      ...
      <nav style={s.navbar}>
        ...
        <div style={s.navUser}>
          ...
          <button style={s.signout} onClick={handleSignOut}>log out</button>
        </div>
      </nav>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, zIndex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" as const }}>
            <span style={{ fontWeight: 800, fontSize: 14, letterSpacing: "0.01em", color: status.hasSubmittedToday ? "#6ee7a0" : "#fca5a5" }}>
            ...
            {status.hasSubmittedToday && (
              <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 99, background: "rgba(74,222,128,0.15)", border: "1px solid rgba(74,222,128,0.3)", color: "#4ade80", letterSpacing: "0.05em" }}>🔥 ON FIRE</span>
            )}
          </div>
            ...
            return (
              <div key={stat.difficulty} className="diff-card" style={{ ...s.diffCard, background: cfg.bg, border: `1px solid ${cfg.border}`, animationDelay: `${i * 0.1}s` }}>
                <div style={s.diffHeader}>
                  ...
                  {beats != null && (
                    <span style={{ fontSize: 12, color: cfg.color, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace" }}>Beats {beats.toFixed(1)}%</span>
                  )}
                </div>
                <div style={s.diffBody}>
                  ...
                  <div style={s.diffStats}>
                    <div style={{ fontSize: 13, color: "#a9a9b5" }}>Solved</div>
                    <div style={{ fontSize: 22, fontWeight: 800, color: cfg.color, fontFamily: "'JetBrains Mono',monospace", lineHeight: 1.1 }}>{stat.count}</div>
                    <div style={{ fontSize: 11, color: "#a9a9b5", marginTop: 4 }}>of {cfg.max.toLocaleString()} total</div>
                    ...
                    <div style={{ fontSize: 11, color: "#a9a9b5", marginTop: 4 }}>{pct.toFixed(1)}% complete</div>
                  </div>
                </div>
              </div>
            );
                  ...
                  return (
                    <div key={i} style={{ marginBottom: 16 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                        ...
                        <span style={{ fontSize: 13, fontWeight: 800, color, fontFamily: "'JetBrains Mono',monospace", padding: "2px 9px", borderRadius: 6, background: color + "15", border: "1px solid " + color + "35" }}>
                          {lang.problemsSolved}
                        </span>
                      </div>
...
<div style={s.card}>
  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap" as const, gap: 10 }}>
    <div>
      ...
      <div style={{ fontSize: 12, color: "#a9a9b5", marginTop: 3 }}>
        {calendar?.totalActiveDays ?? "—"} active days · {calendar?.streak ?? "—"} day max streak
      </div>
    </div>
    <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
      <span style={{ fontSize: 11, color: "#a9a9b5" }}>Less</span>
      {HEAT_COLORS.map((c, i) => (
        <div key={i} style={{ width: 11, height: 11, borderRadius: 2, background: c, border: i === 0 ? "1px solid #1e1e2a" : "none" }} />
      ))}
      <span style={{ fontSize: 11, color: "#a9a9b5" }}>More</span>
    </div>
  </div>
  {!calendar
    ...
    : (() => {
        ...
        heatWeeks.forEach((week, wi) => {
          ...
          if (firstReal) {
            ...
            const prevWeekFirstReal = wi > 0 ? heatWeeks[wi - 1].find(d => d !== null) : null;
            const prevMonth = prevWeekFirstReal ? new Date(prevWeekFirstReal.date).getMonth() : -1;
            if (month !== prevMonth) {
            ...
            <div style={{ minWidth: "max-content" }}>
              ...
              <div style={{ display: "flex", gap: 3, marginBottom: 4, paddingLeft: 0 }}>
                {heatWeeks.map((_, wi) => {
                  ...
                  return (
                    <div key={wi} style={{ width: 11, fontSize: 9, color: "#6b6b72", fontFamily: "'JetBrains Mono',monospace", overflow: "visible", whiteSpace: "nowrap" as const, flexShrink: 0 }}>
                      {ml ? ml.label : ""}
                    </div>
                  );
                })}
              ...
              <div style={{ display: "flex", gap: 3 }}>
                {heatWeeks.map((week, wi) => (
                  <div key={wi} style={{ display: "flex", flexDirection: "column", gap: 3,color:"#6b6b72" }}>
                    {week.map((day, di) => {
                      ...
                      return (
                        <div key={di}
                          ...
                          style={{
                            ...
                            background: HEAT_COLORS[lvl], cursor: "default",
                            border: lvl === 0 ? "1px solid #1e1e2a" : "none",
                      ...
                      {contest.history.slice(-5).reverse().map((h, i) => (
                        <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                          ...
                          <span style={{ fontSize: 12, color: "#fb923c", fontFamily: "'JetBrains Mono',monospace", fontWeight: 700, flexShrink: 0 }}>
                            {Math.round(h.rating)} <span style={{ color: "#6b6b72", fontWeight: 400 }}>#{h.ranking}</span>
                          </span>
                        </div>
                      ))}
                    </div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  navbar:      { position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 28px", height: 56, background: "rgba(10,10,15,0.9)", borderBottom: "1px solid rgba(255,255,255,0.07)", backdropFilter: "blur(20px)", width: "100%",color:'#e8f808' },
  ...
  navSub:      { fontSize: 10, color: "#d3d3deba", fontFamily: "'JetBrains Mono',monospace" },
  signout:     { padding: "5px 12px", background: "rgb(238, 10, 10)", border: "1px solid rgba(0, 0, 0, 0.07)", borderRadius: 7, color: "#fdfdfd", fontSize: 12, cursor: "pointer", fontFamily: "'Outfit',sans-serif", marginLeft: 4 },
  ...
  kpiLabel:    { fontSize: 10, fontWeight: 700, textTransform: "uppercase" as const, letterSpacing: "0.1em", color: "#6b6b72", marginBottom: 4 },
  kpiSub:      { fontSize: 11, color: "#6b6b72" },
  ...
  cardTitle:   { fontSize: 14, fontWeight: 700, color: "#6b6b72", marginBottom: 16, letterSpacing: "0.02em" },
  empty:       { fontSize: 13, color: "#6b6b72", padding: "20px 0" },
  ...
  badgeLabel:  { fontSize: 8.5, color: "#6b6b72", textAlign: "center" as const, lineHeight: 1.2 },
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media (max-width: 700px) { .reminder-label { display: none !important; } }
